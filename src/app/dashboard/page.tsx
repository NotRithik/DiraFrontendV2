...
import { ConnectButton } from "@/components/ConnectButton"; // Import the new component
import { useDira } from "@/context/DiraContext";
import Decimal from "decimal.js";

const HamburgerIcon = () => (
    <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        <rect x="2" y="3" width="20" height="4" /><rect x="2" y="10" width="20" height="4" /><rect x="2" y="17" width="20" height="4" />
    </svg>
);
...
const calculateHealth = (collateral: number, dira: number, omPrice: number, liquidationRatio: number, safeRatio: number) => {
    ...
    if (debtValue.isZero()) return { ratio: Infinity, percentage: 100 };
    ...
    return { ratio: currentRatio.toNumber(), percentage: Math.max(0, Math.min(100, percentage.toNumber())) };
};
...
export default function DashboardPage() {
    ...
    const handleApplyCollateral = () => {
        ...
        if (diff.isPositive()) lockCollateral(diff.toNumber());
        else unlockCollateral(diff.abs().toNumber());
    };

    const handleApplyDira = () => {
        const diff = new Decimal(sliderDiraValue).sub(confirmedMintedDira);
        if (diff.isZero()) return;
        if (diff.isPositive()) mintDira(diff.toNumber());
        else returnDira(diff.abs().toNumber());
    };
