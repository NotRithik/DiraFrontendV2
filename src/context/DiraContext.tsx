...
export function DiraProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    // setIsLoading(true); 
    try {
      // Fetch OM balance from the wallet
      ...
      // Avoid spamming toasts on background refresh failures
  ...
  useEffect(() => {
    if (isConnected) {
        fetchData(); // Fetch once on connect
  ...
  const executeContract = async (message: ExecuteMsg, funds?: Funds) => {
    ...
    try {
        ...
        await fetchData() // Refresh data immediately after success
    } catch (error) {
  ...
  const connectWallet = () => {
    checkWalletConnection(() => {
        // This is a post-connect action for the generic button, which can be empty.
    });
  };
  ...
  const returnDira = async (amount: number) => {
    ...
    const increaseAllowanceMsg = {
      increase_allowance: {
        ...
        amount: new Decimal(amount).mul(1e6).toFixed(0),
        expires: { never: {} },
      },
    }
    ...
    try {
      const signingClient = await getSigningClient()
      if (!signingClient) throw new Error("Failed to get signing client.")
